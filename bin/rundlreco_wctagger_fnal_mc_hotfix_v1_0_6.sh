...
echo "<<< CHECKING TO SEE IF THE FILE IS GOOD >>>"
py_check_script="
import sys,os
import ROOT as rt

f = rt.TFile(\"merged_dlreco.root\")
t = f.Get(\"_recoTree\")
try:
     nentries = t.GetEntries()
     print \"seems ok. with entries=\",nentries
except:
     print \"seems bad\"
     nentries = None
...
python -c "$py_check_script"
ret=$?
echo "check status returned: "$ret
if [ $ret -eq 0 ];then
    echo "File after hadd is good"
else
    echo "File after hadd is bad"
    exit 1
fi
