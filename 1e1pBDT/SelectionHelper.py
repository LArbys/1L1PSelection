...
class BDTensemble:
    ...
    def __init__(self,tag,BDTnumlist,oldBDT=False):
        ...
        if oldBDT:
          bdtsavedir = '/home/nwkamp/Research/MicroBooNE/1e1pSelectionOld/'
          self.tag = 'old'
          self.BDTnumlist = [0]
          self.bdt[1][0] = pickle.load(open(bdtsavedir+'BDT_1e1p_Run1_6-2-20.pickle','rb'))
          self.bdt[2][0] = pickle.load(open(bdtsavedir+'BDT_1e1p_Run3_6-2-20.pickle','rb'))
          self.bdt[3][0] = pickle.load(open(bdtsavedir+'BDT_1e1p_Run3_6-2-20.pickle','rb'))
        for r in [1,2,3]:
          for b in BDTnumlist:
              try:
